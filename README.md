# Walmart Sales Analytics Dashboard

A comprehensive Power BI dashboard analyzing Walmart's retail sales performance across multiple dimensions including customer segments, product categories, geographical locations, and temporal trends for the year 2023. This dashboard provides end-to-end sales analytics for Walmart's home improvement and construction product lines, offering insights into customer behavior, product performance, discount strategies, and store-level metrics to drive strategic business decisions.

## Key Performance Indicators

### Overall Metrics (2023)
- **Total Sales:** 1.90M
- **Total Quantity Sold:** 7,529 units
- **Average Discount:** 3.67%
- **Date Range:** January 1, 2023 - December 31, 2023

## Sales Performance Analysis

### Monthly Sales Trends
Sales remained relatively stable throughout 2023 with notable patterns:
- **Peak Month:** December (370K) - holiday season surge
- **Strongest Quarter:** Q1 (Jan-Mar: ~340K average)
- **Weakest Month:** November (260K)
- **Average Monthly Sales:** 316K

### Discounted vs. Regular Sales
Monthly breakdown shows consistent discount strategy:
- Average discounted sales: approx. 150K per month
- Discount penetration: approx. 47% of total sales
- Highest discounted sales: January-March (170K)
- Lowest discounted sales: June (130K)

## Customer Segmentation

The dashboard tracks four distinct customer segments:
- **DIY Homeowner** - Individual home improvement enthusiasts
- **Professional Contractor** - Trade professionals and builders
- **Property Manager** - Commercial property maintenance
- **Renovation Enthusiast** - Home renovation specialists

## Product Performance

### Top 5 Products by Sales
1. **Power Tools:** 93K
2. **Hardwood:** 81K
3. **Dryers:** 78K
4. **Workbenches:** 77K
5. **Landscaping Supplies:** 74K

### Bottom 5 Products by Sales
1. **Outdoor Furniture:** 32K (lowest)
2. **Drywall:** 41K
3. **Tile:** 43K
4. **Bulbs:** 43K
5. **Cement:** 44K

### Product Insights
- Power tools dominate with 2.9x sales of lowest performer
- Construction materials (cement, drywall, tile) underperform
- High-ticket items (power tools, hardwood) drive revenue

## Store Performance

### Top 3 Stores by Sales
1. **Seattle** - Leading market
2. **Los Angeles** - Strong metropolitan performance
3. **Chicago** - Consistent third-place performer

## Tech Stack
- **Python** - To do data preprocessing
- **MSSQL** - Storing the data into the database
- **Power BI Desktop** - Dashboard development and visualization
- **DAX (Data Analysis Expressions)** - Custom measures and calculations
- **Interactive Filtering** - Dynamic customer segment analysis

## Dashboard Features

### Visualization Components
- **KPI Cards:** Quick-glance metrics (sales, quantity, discount)
- **Time Series Analysis:** Monthly trend tracking with dual metrics
- **Customer Segmentation View:** Sales by customer type
- **Product Ranking:** Top/bottom performers identification
- **Geographic Analysis:** Store-level performance comparison
- **Discount Impact Analysis:** Regular vs. discounted sales comparison

### Interactive Elements
- Date range filtering
- Customer segment filtering
- Drill-down capabilities
- Dynamic measure switching
- Cross-filtering across visuals

## Business Insights

### Revenue Optimization
- December shows 42% increase over November - capitalize on Q4
- Q1 maintains strong performance - focus marketing efforts
- Summer months (Jun-Aug) show decline - consider promotional campaigns

### Product Strategy
- Power tools and hardware drive revenue - expand inventory
- Building materials underperform - review pricing/positioning
- Top 5 products account for approx. 400K (21% of total sales)

### Discount Effectiveness
- 3.67% average discount maintains healthy margins
- ~50% discount penetration balances acquisition and profitability
- Discounted sales remain consistent - indicates effective pricing strategy

### Geographic Focus
- Seattle, LA, Chicago dominate - allocate resources accordingly
- Top 3 stores likely drive 40-50% of total revenue
- Opportunity to analyze underperforming locations

## Use Cases

This dashboard enables:
- **Sales Managers:** Track performance against targets
- **Product Teams:** Identify inventory optimization opportunities
- **Marketing:** Design targeted campaigns by customer segment
- **Store Operations:** Benchmark store performance
- **Executive Leadership:** Strategic planning and resource allocation
- **Finance:** Revenue forecasting and discount impact analysis


## Project Highlights

This project demonstrates:
- Retail analytics expertise
- Multi-dimensional data analysis
- Customer segmentation strategy
- Product portfolio management
- Geographic performance analysis
- Time-series trend identification
- Business intelligence storytelling

## Data Dimensions

- **Temporal:** Daily/monthly sales patterns (2023)
- **Customer:** Four distinct segments
- **Product:** 10+ product categories tracked
- **Geographic:** Multi-city store network
- **Financial:** Sales, discounts, quantity metrics

---

**Project Type:** Retail Analytics, Business Intelligence Dashboard  
**Tools:** Python, MSSQL, Power BI, DAX  
**Domain:** Retail Sales, Customer Analytics   
**Focus Areas:** Sales Optimization, Product Performance, Customer Segmentation  
**Skills Demonstrated:** BI Development, Data Visualization, Retail Strategy Analysis
